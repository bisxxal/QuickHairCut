...
const TrackPage = () => {
  ...
  useEffect(() => {
    ...
    const amount = data?.data.reduce((acc: { amount: number, date: string }[], curr: { amount: number, completedAt: string, }) => {
      let currDate = moment(curr.completedAt).format(' Do MMMM YYYY')
      const existing = acc.find(i => i.date === currDate)
      if (existing && curr.amount !== null) {
        existing.amount = curr.amount
      }
      else {
        if (curr.amount !== null) {
          acc.push({ amount: curr.amount, date: currDate })
        }
      }
      return acc
    }, [])
