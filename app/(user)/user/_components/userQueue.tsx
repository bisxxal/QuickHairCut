...
const UserQueue = ({ lat, long }: { lat: number, long: number }) => {
    ...
    return (
    <>
    <h1 className='pt-10 text-xl max-md:text-lg font-medium'>{!lat && !long ? 'Get you Location to find the nearest shop' : 'Showing nearby Shops'}</h1>
    <div className='mt-10 flex flex-wrap  gap-3'>
        {
            ...
            return (
                <div key={index} className='p-3 max-md:text-sm flex flex-col gap-2 justify-between card bordercolor rounded-3xl max-md:w-[220px] max-md:h-[250px] w-[360px] h-[220px]'>
                    ...
                    {
                        userQueueIndex !== -1 ? (
                            <p className='bg-green-400/40 whitespace-nowrap p-2 rounded-4xl px-3 cursor-default border border-green-500 text-green-600'>
                    ...
                    <Link href={`${shop.gmapLink}`} className=' border border-[#33335b7e] text-white  bg-[#6e56cfcc] px-2 py-2 whitespace-nowrap rounded-4xl flex  gap-0.5 items-center'><Navigation size={18}/>View in Google Map</Link>
                    </div>
                </div>
            )
        })
